...
<xsl:stylesheet xmlns:prop="http://saxonica.com/ns/html-property"
    ...
    <xsl:template match=".[. instance of map(*)]">
        ...
        <xsl:processing-instruction name="xml-model">
            href="https://cdn.jsdelivr.net/gh/EmmaLbrs/Library@main/rngTbt.rng" type="application/xml" schematypens="http://relaxng.org/ns/structure/1.0"            
        ...
        <TEI xmlns="http://www.tei-c.org/ns/1.0">
            ...
            <standOff>
                <xsl:if test="array:size(?Places) != 0">
                    <listPlace>
                        <xsl:apply-templates select="?Places" mode="places"/>
                    </listPlace>
                </xsl:if>
                <xsl:if test="array:size(?Persons) != 0">
                    <listPerson>
                        <xsl:apply-templates select="?Persons" mode="persons"/>
                    </listPerson>
                </xsl:if>
            </standOff>
            <facsimile>
                <xsl:variable name="urlLink">
                    <xsl:value-of
                        select="substring-after(substring-before(?ImageLink, '&#34;,'), '{&#34;@id&#34;:&#34;')"
                    />
                </xsl:variable>
                <graphic>
    ...
    <xsl:template match=".[. instance of map(*)]" mode="places">
        <place>
            <xsl:attribute name="xml:id">
                <xsl:value-of select="concat('place_', ?PlaceId)"/>
            </xsl:attribute>
            ...
            <xsl:if test="?WikidataId">
                <idno type="Wikidata">
                    <xsl:value-of
                        select="concat('https://www.wikidata.org/wiki/', fn:normalize-space(?WikidataId))"
                    />
                </idno>
            </xsl:if>
        </place>
    ...
    <xsl:template match=".[. instance of map(*)]" mode="persons">
        <person>
            <xsl:attribute name="xml:id">
            ...
            <persName>
                <xsl:if test="not(?FirstName = 'NULL')">
                    <forename>
                        <xsl:value-of select="?FirstName"/>
                    </forename>
                </xsl:if>
                <xsl:if test="not(?LastName = 'NULL')">
                    <surname>
            ...
            <birth>
                <xsl:if test="not(?BirthDate = 'NULL')">
                    <date>
                        <xsl:value-of select="?BirthDate"/>
                    </date>
                </xsl:if>
                <xsl:if test="not(?BirthPlace = 'NULL')">
                    <placeName>
            ...
            <death>
                <xsl:if test="not(?DeathDate = 'NULL')">
                    <date>
                        <xsl:value-of select="?DeathDate"/>
                    </date>
                </xsl:if>
                <xsl:if test="not(?DeathPlace = 'NULL')">
                    <placeName>
                        <xsl:value-of select="?DeathPlace"/>
                    </placeName>
                </xsl:if>
            </death>
            <xsl:if test="not(?Description = 'NULL')">
                <note>
...
<!--        <xsl:choose>
            <xsl:when test="local-name(following-sibling::node()[1]) = 'br'">
                2
            </xsl:when>-->
        <!--</xsl:choose>-->
        <lb/>
